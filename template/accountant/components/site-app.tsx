...
const SERVICE_ICONS = [Calculator, FileText, Shield, Users, Clock, Mail]
...
export function SiteApp() {
  ...
  return (
    <div className="min-h-screen bg-white">
      {site.meta.preview && (
        <div className="bg-amber-100 text-amber-950 text-center text-sm py-2 px-4">
          Preview / mockup — not the live website
        </div>
      )}
      {page === "about" ? <AboutView site={site} /> : page === "contact" ? <ContactView site={site} /> : <HomeView site={site} />}
    </div>
  )
}
...
function HomeView({ site }: { site: SiteConfig }) {
  const b = site.business
  const c = site.copy
  return (
    <>
      ...
      <section className="px-8 py-20 max-w-6xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            {c.heroSub ? (
              <p className="text-lg text-emerald-800/70 mb-8 leading-relaxed">{c.heroSub}</p>
            ) : null}
        ...
        <section id="services" className="px-8 py-20 max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-emerald-700/70 max-w-2xl mx-auto">
              {b.category
                ? `${b.category} services${b.area ? ` for ${b.area}` : ""}`
                : "How we can help"}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {site.reviews.map((review) => (
              <div key={`${review.author}-${review.text.slice(0, 24)}`} className="bg-white border border-emerald-100 rounded-2xl p-6 shadow-sm">
                ...
                {review.relativeTime ? (
                  <p className="text-sm text-emerald-600/70 mt-1">{review.relativeTime}</p>
                ) : null}
              </div>
            ))}
          </div>
...
function AboutView({ site }: { site: SiteConfig }) {
  const b = site.business
  const c = site.copy
  return (
    <>
      ...
      <section className="px-8 py-16 max-w-5xl mx-auto text-center">
        ...
        {c.about ? (
          <p className="text-xl text-emerald-800/70 max-w-2xl mx-auto leading-relaxed">{c.about}</p>
        ) : null}
      </section>
      {c.faq.length > 0 && (
...
function ContactView({ site }: { site: SiteConfig }) {
  const b = site.business
  return (
    <>
