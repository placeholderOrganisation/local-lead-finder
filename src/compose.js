...
async function liveOutreach(lead, profile, apiKey) {
  ...
  const generatedAt = new Date().toISOString();
  return normalizeLive(parsed, ctx, model, generatedAt);
}
...
Rules:
- Speak customers and revenue, NEVER raw tech. Forbidden: viewport, LCP, HTTP 403, HTTP 500, meta tags, "SEO audit", TLS, "mobile-friendly tag". Translate to money: "customers on phones can't tap to call you", "people searching Google land on a competitor".
- Segment by website:
...
- If needsVerification is true: do NOT say the site is down, broken, offline, or returning errors. Softer verify-first copy. pitchedAngle = "verify-first".
- Inject proof + local + name from the sender profile. Sign the email with senderName. Mention localArea. Include portfolioUrl if present. If calendarUrl is present, offer it as an optional CTA. If a mockupUrl is present, the email CTA points at it.
...
function normalizeLive(parsed, ctx, model, generatedAt) {
...
function claimsDown(out) {
  ...
  return /\b(site|website|page)\b.{0,24}\b(down|broken|offline|unreachable|dead)\b/.test(blob)
...
function contextOf(lead, profile) {
  ...
  return {
    ...
    lighthouse,
    mockupUrl,
    senderName: profile.senderName || "me",
    localArea: profile.localArea || "your area",
...
function noSite(ctx) {
  ...
  const moneyPitch =
    `${business} is invisible to anyone who doesn't already know them — Google sends those customers to a competitor who looks more established. A simple site in ${localArea} captures the work referrals miss.`;
  ...
  return {
    moneyPitch,
    emailDraft: {
      subject: `${business} — a 2-minute thought from a ${localArea} neighbour`,
      body:
        `Hi — I'm ${senderName}. I help ${localArea} businesses pick up the customers who never get a referral.\n\n` +
...
function hasSite(ctx) {
  ...
  const scoreLine = Number.isFinite(score)
    ? ` Your site scores ${Math.round(score)}/100 on mobile — that's people tapping away before they ever call.`
  ...
  const proof = portfolioUrl ? `\n\nRecent ${localArea} work: ${portfolioUrl}` : "";
  ...
  return {
    ...
    emailDraft: {
      ...
      body:
        `Hi — ${senderName} here, I build sites for ${localArea} businesses.\n\n` +
        `${moneyPitch}\n\n${cta}` + proof + cal + `\n\n${senderName}`,
    },
    phoneScript:
      `Hi, ${business}? ${senderName} in ${localArea}. I looked at how you show up on a phone — people are bouncing before they call. I put a simple mock homepage together with your name on it so you can compare. Worth a 2-minute look?`,
...
function verifyFirst(ctx) {
  const { business, senderName, localArea, portfolioUrl, calendarUrl } = ctx;
  const moneyPitch =
    `I haven't verified ${business}'s site myself yet, so I'm not going to claim anything about it. The conversation is how you show up next to ${localArea} competitors — and whether a clearer first impression would win more of the customers who find you on their phone.`;
  ...
  return {
    ...
    emailDraft: {
      ...
      body:
        `Hi — I'm ${senderName} in ${localArea}. I help local businesses turn phone-searchers into booked jobs.\n\n` +
    ...
    phoneScript:
      `Hi, is this ${business}? ${senderName} — I help ${localArea} businesses pick up customers from Google. I'm not calling with a diagnosis; I haven't walked your site with you yet. I did mock a simple homepage with your name on it so you can see a direction. Open to a 2-minute look before anyone makes claims?`,
    pitchedAngle: "verify-first",
  };
}

function withStubs(assets, ctx) {
  const { business, senderName } = ctx;
  return {
    ...assets,
    whatsapp: `Hi ${business}, ${senderName} here — I mocked a simple homepage with your name on it. Mind if I send the link?`,
    ...
    linkedin: `Hi — I work with local ${ctx.localArea} businesses on how they show up when someone Googles them. I put together a short homepage mock for ${business} if useful.`,
  };
}
